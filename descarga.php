<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>La company</title>

    <link rel="stylesheet" href="vistas/modulos/assets/estilo.css">
    <!-- Resources -->
    <!-- <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-9ndCyUaIbzAi2FUVXJi0CjmCapSmO7SnpJef0486qhLnuZ2cdeRhO02iuK6FUUVM" crossorigin="anonymous"> -->
    <link rel="stylesheet" href="vistas/modulos/assets/bootstrap.min.css">
</head>
<body>
    <div class="wrapper">
    <div class="container-fluid" style="padding:0;">
        <div class="row">
            <div class="col-2">
                <?php
                    include "vistas/modulos/aside.php"
                ?>
            </div>
            <div class="col-10">
                <div class="content-wrapper">
                  <h1>Descarga</h1>
                  <P>Esta aplicación de ejemplo puede ser utilizada como base para preparar su proyecto final.<br />Puede descargar los archivos desde <a href="proyecto.zip">aquí.</a></P>

                </div>    
            </div>
        </div>
    </div>
    </div>
</body>
</html>